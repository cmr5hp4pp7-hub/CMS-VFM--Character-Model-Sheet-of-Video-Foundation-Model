...
## Asset Management & Directory Permissions

When using the in-browser HTML specification editors (e.g., Character Design Sheet, Environment Design Sheet) to upload local files or import URLs:

1. **Default Asset Directory**:
   All imported media assets (images, videos, audios) are automatically copied and saved to the local asset directory within the project folder:
   `Settings&Asset Files/Asset Files`
   All HTML specification sheets resolve and index resources using relative paths based on this folder.

2. **Browser Directory Permission Dialog**:
   - Because of browser security sandbox restrictions, when you import media for the first time, a system dialog will pop up stating: *"Select the location where this website can save changes"* .
   - **Important**: You **MUST** select the `Settings&Asset Files/Asset Files` folder inside your local `CMS-VFM` project directory and grant write permission. This allows the page to write files to your disk, making sure they are saved in the correct place and previewed successfully.
